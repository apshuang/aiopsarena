...
port_forward() {
  ...
  pid=$(ps -aux | grep -w "kubectl port-forward svc/$service_name -n $namespace --address 0.0.0.0 $local_port:$remote_port" | grep -v 'grep' | awk '{print $2}');
  if [ -n "$pid" ] ; then
    echo "Found process:" $pid
    kill -9 $pid
  fi

  nohup kubectl port-forward svc/$service_name -n $namespace --address 0.0.0.0 $local_port:$remote_port > $log_file &
}


# 转发prometheus的端口，本地端口为9090，远程端口为9090，日志文件为prometheus.log
check_port 9090
port_forward prometheus-server istio-system 9090 80 prometheus.log

# 转发elasticsearch的端口，本地端口为9200，远程端口为9200，日志文件为elasticsearch.log
check_port 9200
port_forward elasticsearch-master observe 9200 9200 elasticsearch.log

# 转发kibana的端口，本地端口为5601，远程端口为5601，日志文件为kibana.log
check_port 5601
port_forward kibana-kibana observe 5601 5601 kibana.log

# 转发grafana的端口，本地端口为3000，远程端口为3000，日志文件为grafana.log
check_port 3000
port_forward grafana observe 3000 3000 grafana.log
